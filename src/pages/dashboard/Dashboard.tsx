...
// 🔥 Firebase
import { getDashboardStats, getOverdueBorrowers } from "../../firebase/services";
...
const Dashboard = () => {
  ...
  const StyledTableCell = styled(TableCell)(({ theme }) => ({
    [`&.${tableCellClasses.head}`]: { backgroundColor: theme.palette.common.black, color: theme.palette.common.white, whiteSpace: "nowrap" },
  ...
  async function loadData() {
    ...
    try {
      ...
      const monthData: any = {};
      monthNames.forEach((m, i) => { monthData[i] = { overdue: 0, borrowed: 0, month: m }; });
      const now = new Date();
      borrowsSnap.forEach((doc: any) => {
        const d = doc.data();
        const borrowDate = d.dateBorrowed?.toDate?.() || (d.dateBorrowed ? new Date(d.dateBorrowed) : null);
        if (borrowDate) {
      ...
      <div className="dashboard_component p-4 max-w-360 mx-auto">
        <div className="header_dashboard_admin flex justify-between items-center">
          <div className="search_logo_and_search_input relative flex-1">
            ...
            <input type="search" className="inp_search outline-none shadow-[0_0_6px_gray] pl-12 pr-4 py-2 rounded-[30px] text-[18px] font-500 sm:w-full md:w-[90%] lg:w-[80%]" placeholder="Search enter..." />
          </div>
        ...
        <div className="section_dashboard_admin mt-5">
          <h1 className="title_admin_dashboard text-[25px] font-600">Admin Dashboard</h1>
          ...
          {loadingStat ? (
            ...
            <div className="amount_block flex justify-between items-center mt-2 sm:flex-col md:flex-row md:flex-wrap gap-4">
              <div className="amount_block_members bg-[#F1E7FF] p-2.5 flex justify-between items-center gap-5 rounded-[10px] sm:w-full md:w-[48%] lg:w-max flex-1">
                <div><h1 className="title">Members</h1><h1 className="amount">{stat.total_members ?? 0}</h1></div>
              ...
              <div className="amount_block_total_books bg-[#E4F5FF] p-2.5 flex justify-between items-center gap-5 rounded-[10px] sm:w-full md:w-[48%] lg:w-max flex-1">
                <div><h1 className="title">Total Books</h1><h1 className="amount">{stat.total_books ?? 0}</h1></div>
              ...
              <div className="amount_block_books_borrowers bg-[#EAFEEF] p-2.5 flex justify-between items-center gap-5 rounded-[10px] sm:w-full md:w-[48%] lg:w-max flex-1">
                <div><h1 className="title">Books Borrowers</h1><h1 className="amount">{stat.active_borrows ?? 0}</h1></div>
                <SecurityUpdateGoodOutlinedIcon style={{ color: "#00FF40", fontSize: "40px" }} />
              </div>
              <div className="amount_block_overdue_books bg-[#FFDADB] p-2.5 flex justify-between items-center gap-5 rounded-[10px] sm:w-full md:w-[48%] lg:w-max flex-1">
                <div><h1 className="title">Overdue Books</h1><h1 className="amount">{stat.overdue_books ?? 0}</h1></div>
                <MdOutlineSecurityUpdateWarning style={{ color: "#FD286F", fontSize: "40px" }} />
              </div>
            </div>
          )}
          ...
          <div className="monthly_borrowing_summary_and_volunteeers_graph_block mt-5 flex flex-col lg:flex-row justify-between gap-8 lg:gap-20">
            <div className="monthly_borrowing_summary_graph_block w-full lg:w-2/3 overflow-x-auto">
              <h1 className="title_monthly_borrowing_summary text-[25px] font-600 mb-4">Monthly Borrowing Summary</h1>
              <BarChart
                dataset={dataset}
                xAxis={[{ dataKey: "month", scaleType: "band" }]}
                series={[
                  { dataKey: "overdue", label: "Overdue", valueFormatter: (v: number | null) => `${v}` },
                  { dataKey: "borrowed", label: "Borrowed", valueFormatter: (v: number | null) => `${v}` },
                ]}
                yAxis={[{ width: 60 }]}
                height={300}
              />
          ...
          <div className="priority_overdue_borrewers_list_block mt-8">
            <h1 className="title_priority_overdue_borrewers text-[25px] font-600 mb-4">Priority Overdue Borrowers</h1>
            <TableContainer>
              <Table aria-label="customized table">
                <TableHead>
                  <TableRow>
                    <StyledTableCell sx={{ minWidth: 150 }}>Full Name</StyledTableCell>
                    <StyledTableCell sx={{ minWidth: 150 }}>Phone Number</StyledTableCell>
                    <StyledTableCell sx={{ minWidth: 200 }}>Book Title</StyledTableCell>
                    <StyledTableCell sx={{ minWidth: 120 }}>Borrow date</StyledTableCell>
                    <StyledTableCell sx={{ minWidth: 120 }}>Due Date</StyledTableCell>
                    <StyledTableCell sx={{ minWidth: 120 }}>Days Overdue</StyledTableCell>
                    <StyledTableCell sx={{ minWidth: 120 }}>Status</StyledTableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {overdueBorrowers.length === 0 ? (
                    <TableRow><TableCell colSpan={7}><h1 className="text-center text-gray-400 py-4">No overdue borrowers 🎉</h1></TableCell></TableRow>
                  ) : (
                    overdueBorrowers.map((borrower: any) => (
                      <StyledTableRow key={borrower.id}>
                        <StyledTableCell>{borrower.borrowerName}</StyledTableCell>
                        <StyledTableCell>{borrower.phoneNumber}</StyledTableCell>
                        <StyledTableCell>{borrower.bookTitle}</StyledTableCell>
                        <StyledTableCell>{borrower.dateBorrowed?.toDate?.()?.toLocaleDateString?.() || borrower.dateBorrowed}</StyledTableCell>
                        <StyledTableCell>{borrower.dueDate?.toDate?.()?.toLocaleDateString?.() || borrower.dueDate}</StyledTableCell>
                        <StyledTableCell>{borrower.daysOverdue}</StyledTableCell>
                        <StyledTableCell style={{ color: "red", fontWeight: "bold" }}>Danger</StyledTableCell>
                      </StyledTableRow>
                    ))
                  )}
                </TableBody>
