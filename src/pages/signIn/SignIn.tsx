...
const SignIn = () => {
  ...
  const slidesData = [
    { id: 1, image: slideImg1, title: "Welcome", description: "You enter the verified login and become an administrator. Good luck to you." },
    { id: 2, image: slideImg2, title: "Dear Admin!", description: "All the features and improvements of the program are in your hands." },
    { id: 3, image: slideImg3, title: "Dear Admin!", description: "All the features and improvements of the program are in your hands." },
  ];
  ...
  function validateForm() {
    ...
    if (!emailInpValue.trim()) {
      ...
      isValid = false;
    } else if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(emailInpValue.trim())) {
  ...
  async function handleSignIn(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (code === "auth/user-not-found" || code === "auth/wrong-password" || code === "auth/invalid-credential") {
        ...
        setErrors({ email: "Too many attempts. Please try again later.", password: "" });
      } else {
            ...
            {slidesData.map((slide) => (
              <SwiperSlide key={slide.id} className="flex items-center justify-center">
                <div className="slide_block slides_block">
                  <img src={slide.image} className="img_slides" alt={slide.title} />
                  ...
                  <div className="text_block absolute bottom-10 left-10 right-10 px-6 py-2 bg-white rounded-xl shadow-2xl">
                    ...
                    <p className="description_admin md:text-[14px] lg:text-[17px] font-400 max-w-175 md:leading-4 lg:leading-4.5">{slide.description}</p>
                  </div>
                </div>
              </SwiperSlide>
            ))}
        ...
        <div className="sign_in_block_2 h-screen flex justify-center items-center w-full sm:flex-col md:flex-row">
          <form className="form_sign_in px-4" onSubmit={handleSignIn}>
            <div className="block_logo_and_title_sign_in_component flex flex-col justify-center items-center">
              ...
              <h1 className="title_sign_in_block_2 text-center text-[#100F14] text-[32px] font-600">Sign In</h1>
            </div>
            ...
            <div className="label_input_email_password_sign_in mt-2 flex flex-col gap-5">
              ...
              <div className="form_sign_in_block_1_email">
                ...
                <TextField
                  id="email" label="Enter your email" variant="outlined" fullWidth
                  error={Boolean(errors.email)} helperText={errors.email}
                  sx={{ marginTop: 1 }}
                  value={emailInpValue}
                  onChange={(e) => { setEmailInpValue(e.target.value); if (errors.email) setErrors((p) => ({ ...p, email: "" })); }}
                />
              </div>

              {/* Password */}
              <div className="form_sign_in_block_2_password">
                <label htmlFor="password" className="label_password text-[#9794AA] text-[16px] font-500 cursor-pointer">Password</label>
                <FormControl fullWidth variant="outlined" sx={{ marginTop: 1 }} error={Boolean(errors.password)}>
                  <InputLabel htmlFor="outlined-adornment-password">Password</InputLabel>
                  <OutlinedInput
                    id="outlined-adornment-password"
                    type={isShowPassword ? "text" : "password"}
                    value={passwordInpValue}
                    onChange={(e) => { setPasswordInpValue(e.target.value); if (errors.password) setErrors((p) => ({ ...p, password: "" })); }}
                    endAdornment={
                      <InputAdornment position="end">
                        <IconButton aria-label={isShowPassword ? "hide password" : "show password"} onClick={handleClickPassword} onMouseDown={handleMouseDownPassword} onMouseUp={handleMouseUpPassword} edge="end">
            ...
            <p className="text-center text-[#8E8E8E] text-[18px] font-400 mt-4">
              ...
              <Link to={"/sign-up"} className="text-[#3A65FF] hover:underline">Sign up</Link>
            </p>
          </form>
        </div>
      </div>
